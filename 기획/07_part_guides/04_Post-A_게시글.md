...
`Post` 엔티티의 CRUD, 좋아요(`PostLike`), 검색·정렬 입구, 어드민 운영.
...
### Request 도메인이 사용
- `requireExists(postId)` — `linked_post_id` 설정 시 검증용
...
- `FileService.saveAndAttach(postId, files, thumbnail)` — File-A
- `FileService.deleteByPost(postId)` — File-A (게시글 삭제 시)
...
### Day 3-6: MVP
- [ ] `create()` 트랜잭션: User 검증 → Post 저장 → Tag findOrCreate → Files saveAndAttach
- [ ] `search()` 쿼리: criteria(`categoryId, tag, q, authorId, teamId, sort`) → Specification 또는 동적 JPQL
